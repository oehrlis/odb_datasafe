...
parse_common_opts() {
  ...
  for (( i=0; i<${#args[@]}; i++ )); do
    case "${args[i]}" in
      -T|--target)        core_need_val args "$i" TARGET; ((++i)) ;;
      -c|--compartment)   core_need_val args "$i" COMPARTMENT; ((++i)) ;;
      -O|--output-type)
        core_need_val args "$i" val; ((++i))
        case "$val" in
          table|json) OUTPUT_TYPE="$val" ;;
          *) core_log_message ERROR "Invalid --output-type '$val' (use: table|json)"; Usage 2 ;;
        esac
        ;;
      --oci-config)       core_need_val args "$i" OCI_CLI_CONFIG_FILE; ((++i)) ;;
      --oci-profile)      core_need_val args "$i" OCI_CLI_PROFILE; ((++i)) ;;
      -l|--log-file)      core_need_val args "$i" SCRIPT_LOG; ((++i)) ;;
      -v|--verbose)       VERBOSE=true ;;
      -d|--debug)         DEBUG=true ;;
      -q|--quiet)         QUIET=true ;;
      --log-level)        core_need_val args "$i" G_LOG_LEVEL; ((++i)); EXPLICIT_LOG_LEVEL=true ;;
      -h|--help)          Usage 0 ;;
      --)                 ((++i)); while (( i < ${#args[@]} )); do positional+=( "${args[i++]}" ); done; break ;;
      -*)                 core_log_message ERROR "Unknown option: ${args[i]}"; Usage 2 ;;
      *)                  positional+=( "${args[i]}" ) ;;
    esac
  done
...
validate_inputs() {
  ...
  [[ -z "${TARGET:-}" ]] && { core_log_message ERROR "Missing mandatory parameter: -T/--target"; Usage 2; }
