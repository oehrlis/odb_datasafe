...
#### 📋 `ds_target_list.sh` - Target Listing & Analysis

- **Count Mode (Default):** Summary by lifecycle state with totals

  ```bash
  ds_target_list.sh                    # Show count summary
  # Output: ACTIVE: 1179, NEEDS_ATTENTION: 7, TOTAL: 1186
  ```

- **Details Mode:** Table, JSON, or CSV output formats

  ```bash
  ds_target_list.sh -D                 # Table format
  ds_target_list.sh -D -f json         # JSON format  
  ds_target_list.sh -D -f csv          # CSV format
  ```
...
#### 🏷️ `ds_target_update_tags.sh` - Tag Management  

- **Pattern-Based Environment Detection:** Derives environment from compartment names

  ```bash
  cmp-lzp-dbso-prod-projects → Environment=prod
  cmp-lzp-dbso-test-projects → Environment=test
  cmp-lzp-dbso-qs-projects   → Environment=qs
  ```
...
#### 📊 `ds_tg_report.sh` - Comprehensive Tag Reports
...
- **Impact:** 9x faster bulk operations (90s → 10s for 9 targets)

#### 🎛️ Wait Control Options
...
- Now accepts **both** compartment names and OCIDs:
  ...
  ```bash
  DS_ROOT_COMP="cmp-lzp-dbso"                          # Name ✅
  DS_ROOT_COMP="ocid1.compartment.oc1..aaa..."        # OCID ✅
  ```
...
- Priority: `$ORADBA_ETC/datasafe.conf` → `etc/datasafe.conf` → script-specific
- Usage:
...
- ✅ Version display correct (0.2.0)
- ✅ Compartment name resolution working
- ✅ OCID detection working
- ✅ Default behavior functional
- ✅ ORADBA_ETC loading confirmed
- ✅ Arithmetic expressions return exit code 0
- ✅ Real refresh operations successful
...
### DS_ROOT_COMP_OCID → DS_ROOT_COMP
...
**Enjoy the improved Data Safe management experience!** 🎉
