...
- Unified action toggle flags:
  - `--action` / `--no-action`
- Updated overview option names:
  - `--status`, `--no-status`, `--no-members`,
    `--truncate-members`, `--no-truncate-members`
- Aligns overview/problem/health workflows for faster triage on large target estates.
...
- Added coverage metrics:
  - SID→CDB coverage count and percentage
  - average PDBs per CDB
  - average targets per SID
- Extended issue summary with SID blast radius (`SID %`).
- Updated issue summary table formatting to health-overview-style aligned
  single-line rows (Issue/Severity/Count/SIDs/SID %/Action).
- Consolidated issue-summary table rendering so `--mode health`,
  `--mode problems`, and `--mode report` now use the same report-style
  columns for comparable triage output.
...
- Added top-SID visibility context (`top 10`, `showing X of Y`) to avoid
  ambiguity when total affected SIDs exceed displayed entries.
- Suppressed empty report sections when no issues are present
  (`NEEDS_ATTENTION breakdown: none`, `Issue summary: none`,
  `Top affected SIDs: none`).

### Target Source Reuse Beyond `ds_target_list.sh`

- Added reusable source helpers in `lib/oci_helpers.sh` for normalized target
  JSON load/save and source collection.
- Added Phase 1 `--input-json` / `--save-json` support for read-only analysis:
  `ds_tg_report.sh`, `ds_find_untagged_targets.sh`, and
  `ds_target_connector_summary.sh`.
- Added Phase 2 `--input-json` / `--save-json` support for
  `ds_target_export.sh` and `ds_target_details.sh`, enabling offline
  export/detail workflows from saved target payloads.
...
- Fixed `lib/common.sh` `error_handler` to preserve original failing exit code before
  disabling ERR trap.
- Removed dead helper functions from `bin/ds_target_list.sh`
  (`list_targets_in_compartment`, `get_target_details`).
- Reduced repeated JSON processing overhead in report generation by consolidating
  repeated `jq` scans in `build_consolidated_report_json` and `show_report_table`.
- Updated common logging defaults/flags in `lib/common.sh`:
  default logging is now quiet (`WARN`), `--verbose` enables `INFO`,
  and `--debug` enables full trace (`TRACE`).
